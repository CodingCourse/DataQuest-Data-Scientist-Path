## 1. If Statements ##

opened_file = open('AppleStore.csv')
from csv import reader
read_file = reader(opened_file)
apps_data = list(read_file)

free_apps_ratings = []
for row in apps_data[1:]:
    rating = float(row[7])
    price = float(row[4])
    if price == 0.0:
        free_apps_ratings.append(rating)

avg_rating_free = sum(free_apps_ratings) / len(free_apps_ratings)
n_free_apps = len(free_apps_ratings)
percentage_free_apps = (n_free_apps / len(apps_data[1:])) * 100

## 2. Booleans ##

a_price = 0
prices = [0, 2, 0, 0, 0]
app_and_price = [['Facebook', 0], ['Instagram', 0], ['Plants vs. Zombies', 0.99],
                 ['Minecraft: Pocket Edition', 6.99], ['Temple Run', 0],
                 ['Plague Inc.', 0.99]]
if a_price == 0:
    print("This is free")
else:
    print('This is not free')
    
free = []
for value in prices:
    if value == 0:
        free.append(value)

n_free_prices = len(free)

free_apps = []
for apps in app_and_price:
    name = apps[0]
    price = apps[1]
    if price == 0.0:
        free_apps.append(name)

print(free_apps)        

## 3. The Average Rating of Non-free Apps ##

non_free_apps_ratings = []
for apps in apps_data[1:]:
    rating = float(apps[7])
    price = float(apps[4])
    if price != 0.0:
        non_free_apps_ratings.append(rating)

avg_rating_non_free = sum(non_free_apps_ratings) / len(non_free_apps_ratings)
print(avg_rating_non_free)

answer = '''It is evident from data that, paid apps are slightly better in rating than that of free apps'''

## 4. The Average Rating of Gaming Apps ##

non_games_ratings = []
for apps in apps_data[1:]:
    rating = float(apps[7])
    genre = apps[11]
    if genre != 'Games':
        non_games_ratings.append(rating)

avg_rating_non_games = sum(non_games_ratings) / len(non_games_ratings)

answer = '''The main cause for gaming apps to have a slightly better rating might be due to the age demographics of the users as well as the fun part of the game.'''


## 5. Multiple Conditions ##

free_games_ratings = []
for apps in apps_data[1:]:
    rating = float(apps[7])
    price = float(apps[4])
    genre = apps[11]
    if price == 0.0 and genre == 'Games':
        free_games_ratings.append(rating)

avg_rating_free_games = sum(free_games_ratings) / len(free_games_ratings)

## 6. The or Operator ##

games_social_ratings = []
for apps in apps_data[1:]:
    rating = float(apps[7])
    genre = apps[11]
    if genre == 'Social Networking' or genre == 'Games':
        games_social_ratings.append(rating)

avg_games_social = sum(games_social_ratings) / len(games_social_ratings)

## 7. Combining Logical Operators ##

free_games_social_ratings = []
paid_games_social_ratings = []
for row in apps_data[1:]:
    rating = float(row[7])
    genre = row[11]
    price = float(row[4])
    
    if genre == 'Social Networking' or genre == 'Games':
        if price == 0:
            free_games_social_ratings.append(rating)
        else:
            paid_games_social_ratings.append(rating)
                
avg_free = sum(free_games_social_ratings) / len(free_games_social_ratings)
avg_non_free = sum(paid_games_social_ratings) / len(paid_games_social_ratings)


## 8. Comparison Operators ##

apps_more_9 = []
for apps in apps_data[1:]:
    if float(apps[4]) > 9.0:
        apps_more_9.append(float(apps[7]))

n_apps_more_9 = len(apps_more_9)
avg_rating = sum(apps_more_9) / n_apps_more_9
n_apps_less_9 = len(apps_data[1:]) - n_apps_more_9

## 9. The else Clause ##

for apps in apps_data[1:]:
    if float(apps[4]) == 0.0:
        apps.append('free')
    else:
        apps.append('non-free')
        
apps_data[0].append('free_or_not')

print(apps_data[:6])

## 10. The elif Clause ##

for apps in apps_data[1:]:
    price = float(apps[4])
    if price == 0.0:
        apps.append('free')
    elif price > 0.0 and price < 20:
        apps.append('affordable')
    elif price >= 20 and price < 50:
        apps.append('expensive')
    else:
        apps.append('very expensive')

apps_data[0].append('price_label')

print(apps_data[:6])